...
const subjectSchema = z.object({
    subjectName: z.string().min(1, "Subject name is required").regex(/^[A-Za-z ]+$/, "Only letters are allowed"),
});
const Subjects = () => {
    ...
    const selectedSubjects = subjectList.slice(startIndex, startIndex + SubjectsPerPage);
    ...
    const handleUpdateSubject = (data) => {
        setSubjectList((prev) => prev.map((sub) => (sub.id === selectedSubject.id ? { ...sub, name: data.subjectName } : sub)));
        ...
        <SidebarProvider style={{ "--sidebar-width": "15rem" }}>
            ...
            <SidebarInset>
                ...
                <main className="flex-1 overflow-auto">
                    <div className="w-full shadow-md shadow-blue-300/30 rounded-lg flex flex-wrap sm:flex-nowrap items-center justify-between px-4 sm:px-8 py-4 gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <Button onClick={() => setAddSubjects(true)} className="bg-blue-600 text-white hover:bg-blue-500 px-4 py-2 rounded-md text-sm flex items-center gap-2">
                        ...
                        <div className="flex items-center border border-blue-300 rounded-lg px-3 py-2 w-full sm:max-w-md">
                            ...
                            <input type="text" placeholder="By Subjects Name..." className="ml-2 w-full outline-none bg-transparent text-sm" />
                        </div>
                    </div>
                    ...
                    <div className="grid w-full grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-6">
                        {selectedSubjects.map((sub) => (
                            <Card key={sub.id} className="w-full max-w-xs sm:max-w-sm mx-auto shadow-md shadow-blue-300/20 rounded-xl">
                                <CardHeader>
                                    <CardTitle className="text-lg font-semibold text-center">Subject : {sub.name}</CardTitle>
                                </CardHeader>
                                <CardFooter className="flex flex-col items-center space-y-2">
                                    <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded-md transition" onClick={() => handleEditSubject(sub)}>
                                        Edit Subject
                                    </Button>
                                </CardFooter>
                            </Card>
                ...
                <Dialog open={EditSubjects} onOpenChange={setEditSubjects}>
                    <DialogContent onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()} className="sm:max-w-[600px] shadow-lg p-6 rounded-lg">
                        <DialogHeader>
                            <DialogTitle className="text-center">Update Subject Name</DialogTitle>
                        </DialogHeader>
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(handleUpdateSubject)} className="space-y-4">
                                ...
                                />
                                <Button type="submit" className="w-full bg-blue-600 text-white">Confirm</Button>
                            </form>
                        </Form>
                    </DialogContent>
                </Dialog>
                    ...
                    >
                        ...
                        <Form {...form}>
                            ...
                            >
                                ...
                                />
                                <Button type="submit" className="w-full bg-blue-600 text-white">Add Subject</Button>
                            </form>
                        </Form>
                    </DialogContent>
                </Dialog>
